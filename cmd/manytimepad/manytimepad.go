...
func mtp() (err error) {
	...
	///Define minLen
	// bIndex stands for byte index
	ct1 := cts[0]
	ct2 := cts[1]

	for i := 0; i < minLen; i++ {
		var msgDiscovered [8]int
		for _, ct1 := range cts {
			for _, ct2 := range cts {
				x := ct1[i]
				y := ct2[i]
				...
				switch {
				case msb(x) >= msb(y):
					mostSignificantBit = msb(x)
				case msb(x) < msb(y):
					mostSignificantBit = msb(y)
				}

				mask := mostSignificantBit - 1

				// For all bits in revCon that are equal to 0 and less then msb
				discoveredBits := (x ^ y) ^ byte(mask)
				for shift := 7; shift != 0; shift-- {
					var mask byte
					mask = 1 << uint(shift)
					if discoveredBits&mask != 0 {
						fmt.Print("x")
					} else {
						fmt.Print(0)
					}
	...
	// For all different combinations of cipher texts; XOR
	// cts[i][fixed] 45
	// XOR all possible combinations of bytes from the cipher texts until all bits in a message byte is found
	// When a whole message byte is found; XOR the message byte with it's cipher text byte on the same offset
	// the resulting value is the value of the key for that offset

	// fmt.Printf("--------\n%s\n%s\n", strconv.FormatInt(int64(math.Pow(2, float64(mostSignificantBit))), 2), strconv.FormatInt(int64(discoveredBits), 2))

	// x ^ y = asdf
	// asdf[0s] = msg[1s]
	// msg[1s] ^ cipher[bits] = key[bits]

	return nil
}

func msb(num byte) (msb int) {
	for num != 0 {
		num >>= 1
		msb += 1
	}
...
// 1 bit per message is enough to find the key
//
// c1 ^ c2 describes the reverse value with their messages when the bits are the same
// (i.e when xor equals 0 on that particular bit). If they are different we don't know
// which message has the 0 bit and vice versa.
//
// Example:
//    k ^ m1 = c1 (00)
//    k ^ m2 = c2 (01)
//    k ^ m3 = c3 (11)
// c1 ^ c2 = sd, 1 bit of information connected to both m1 and m2 discovered
// 00 ^ 01 = 01, message m1 = 1x, m2 = 1x
// 01 ^ 11 = 10, message m2 = x1, m3 = x1
// cn ^ mn = key
//
// m2   c2   key
// 11 ^ 01 = 10
